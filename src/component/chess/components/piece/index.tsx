...
const Piece = React.memo(
  React.forwardRef<ChessPieceRef, PieceProps>(
    ({id, startPosition, square, size, viewStyle}, ref) => {
      ...
      const wrappedOnMoveForJSThread = useCallback(
        ({move}: {move: Move}) => {
          onMove(move.from as any, move.to as any);
        },
        [onMove],
      );
      ...
      const moveTo = useCallback(
        (from: Square, to: Square) => {
          return new Promise<Move | undefined>(resolve => {
            const move: any = validateMove(from, to);
      ...
      const movePiece = useCallback(
        (to: Square) => {
          if (orientation === 'w') {
            ...
            const x =
              offsetX.value * Math.cos(angle) - offsetY.value * Math.sin(angle);
            const y =
              offsetY.value * Math.cos(angle) + offsetX.value * Math.sin(angle);
      ...
      const globalMoveTo = useCallback(
        (move: Move) => {
          const refsData = refs as any;
          refsData?.current?.[move.from].current.moveTo?.(move.to);
        },
        [refs],
      );
      ...
      const gesture = Gesture.Pan()
        ...
        .onBegin(() => {
          offsetX.value =
            orientation === 'w' ? translateX.value : -translateX.value;
          offsetY.value =
            orientation === 'w' ? translateY.value : -translateY.value;
